...
**Site settings** controls names, motto, introduction, About copy, history, official contact email, social links, affiliation, league logo and patron image, footer, leadership term, and search metadata. In the About and history fields, blank lines separate paragraphs, and lines beginning with a hyphen make the diocesan chapter list (indent chapters beneath each diocese by two spaces). The short About and patron saint introduction fields appear immediately; their history and full biography open with separate Learn more controls. In the patron biography field, blank lines separate paragraphs. Enter patron dates one per line as `Label: value` (for example, `Baptized: April 18, 1676`). Editor notes are not rendered. Use these notes for content awaiting confirmation. Navigation and general interface labels live in the components.

Images uploaded through Keystatic are converted to WebP in the browser and saved in `src/assets/images/`. PNG, JPEG, AVIF, and WebP inputs are supported; if a browser cannot convert a file, upload a WebP version instead. Resize unusually large source artwork before uploading it. The public visitor receives appropriately sized WebP images with explicit dimensions. The tiny PNG favicon and Apple touch icon remain PNG for icon compatibility. The hero banner and full coat of arms are separate WebP assets in `src/assets/images/`; their placement is defined in `src/components/Hero.astro`. Vietnamese and Latin font subsets are self-hosted; no Google Fonts request is made by visitors.
