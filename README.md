...
Choose **Chapters / Các Đoàn** or **Leadership / Ban Thường Vụ**, open an entry, and edit the fields. Lower display-order numbers appear first. Upload portraits or chapter crests only if available and approved; add an image description. No invented portraits or placeholder people are displayed. Change the leadership term under **Site settings**.

The chapter section opens with an interactive map of southern Ontario and a province overview. Hover, focus, or tap a numbered marker to show its chapter; **List view** preserves the full chapter directory. The map uses the same managed chapter names, parish details, and links as the list. **Map latitude** and **Map longitude** locate the approximate community centre, not an exact meeting address. Leave them empty to keep a chapter only in the list. Chapters outside the displayed southern Ontario area also remain in the list. The seven existing callout positions in `ChapterMap.astro` separate nearby markers; update those positions if a chapter moves substantially. Without JavaScript, the full list is shown automatically.
...
**Site settings** controls names, motto, introduction, About copy, history, official contact email, social links, affiliation, hero and patron images, footer, leadership term, and search metadata. Editor notes are not rendered. Use these notes for content awaiting confirmation. Navigation and general interface labels live in the components.
